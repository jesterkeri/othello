 ...
 */
import { validateBindingRecord, BINDING_MAX_AGE_DAYS, BINDING_MAX_SKEW_MS } from "../ops/issuer.ts";
...
const cases: { name: string; record: unknown; expect: string | null }[] = [
  ...
  {
    name: "verifiedAt is in the future, which would make the age negative",
    record: { ...good, verifiedAt: "2099-01-01T00:00:00.000Z" },
    expect: "is in the future",
  },
  {
    name: "verifiedAt is inside the skew allowance",
    record: { ...good, verifiedAt: new Date(Date.now() + BINDING_MAX_SKEW_MS / 2).toISOString() },
    expect: null,
  },
  {
