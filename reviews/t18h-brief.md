# T18h review brief (T18d round 4): r3's fixes, member actions, Buy in place, adversary pass 5
...
| tests | `t18g-member-tools-adversary`, `t18g-swap-amount-adversary`, `app-actions` (+5 bankrun), `app-live-guards` (+2), `app-swap` (11), `app-stock-search` (3), `app-portfolio-exact` (+1, Codex's case), `t18f-shown-tokens-adversary`, `t18f-powers-tile-contrast-adversary`; three LiveCircle render harnesses count five useState calls |

## 3. Already attacked

Adversary pass 5 on T18g `f5f0e42` found two defects, fixed in `47c85a9` with its tests: member tools
were enabled for a configured seat that never joined (no Member account; now the joined bit is
required), and /api/swap rounded or reinterpreted amounts ("1e2", "0x10", "1.0000005"; now only a
plain decimal with at most 6 places, converted with string arithmetic, and the panel sends what was
typed). The relay now also scrubs any long piece of the RPC URL, so a key echoed alone cannot leak.
...
cd /home/hr/myvscode_linux/othello-t18h
git branch --show-current   # must print task/T18h-final; do not check out anything else
